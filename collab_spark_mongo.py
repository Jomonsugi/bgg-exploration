...
def df_predict(df_test, optimized_model):
    ...
    # (full_train, full_test) = ugr_df.randomSplit([0.8, 0.2], seed=0L)
    # als = ALS(rank = rank, maxIter = maxIter, regParam = lmbda, userCol = "user_id", itemCol="game_id", seed = seed, ratingCol="rating")
    # optimized_model = als.fit(full_train)

    # print(full_test.take(3))
...
def predict_for_user_rdd(ugr_rdd, optimized_model, username="jomonsugi"):
    client = MongoClient()
    ...
    try:
        ...
        return
    user_ugr = ugr_rdd.filter(lambda row: row[0] == idn)
    # print("user_ugr_df")
    # print(type(user_ugr_df))
    # print(user_ugr_df.take(1))
    ...
    user_unrated_games = (ugr_rdd.filter(lambda x: x[1] not in rated_game_ids_lst).map(lambda x: (idn, x[1])))
    # print("user_unrated_games:", user_unrated_games.take(3))
...
if __name__ == '__main__':
    ...
    # predict_for_user_rdd(ugr_rdd, optimized_model, username="ahalm")
